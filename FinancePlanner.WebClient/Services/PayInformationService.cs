...
public class PayInformationService : IPayInformationService
{
    ...
    public async Task<PayInformationResponse> GetPay(string userId)
    {
        ...
        string requestUrl = $"{_config.GetSection("Clients:ApiGatewayClient:GetPayInformation").Value}/{userId}";
    ...
    {
        HttpClient httpClient = _httpClientFactory.CreateClient(_config.GetSection("Clients:ApiGatewayClient:ClientName").Value);
        string requestUrl = $"{_config.GetSection("Clients:ApiGatewayClient:SavePayInformation").Value}";
    ...
    {
        HttpClient httpClient = _httpClientFactory.CreateClient(_config.GetSection("Clients:ApiGatewayClient:ClientName").Value);
        string requestUrl = $"{_config.GetSection("Clients:ApiGatewayClient:UpdatePayInformation").Value}/{userId}/{payId}/{incomeId}";
        PayInformationResponse? response = await httpClient.Post<PayInformationRequest, PayInformationResponse>(request, requestUrl);
        return response ?? default;
    }
    ...
    {
        HttpClient httpClient = _httpClientFactory.CreateClient(_config.GetSection("Clients:ApiGatewayClient:ClientName").Value);
        string requestUrl = $"{_config.GetSection("Clients:ApiGatewayClient:DeletePayInformation").Value}/{userId}/{payId}/{incomeId}";
